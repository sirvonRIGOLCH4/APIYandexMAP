...
class Window(QMainWindow, Ui_MainWindow):
    ...
    def find_organization(self, coords_mouse):
        ...
        self.lan_temp = self.coords[1] + self.delta_y * 0.0000428 * math.cos(math.radians(self.coords[1])) * math.pow(2,
                                                                                                                      15 - self.zoom)
        ...
        self.organization = geospn.find_org(self.zapros)
        if not self.organization:
            return
        self.organization_name = self.organization["properties"]["CompanyMetaData"]["name"]
        self.organization_coords = self.organization["geometry"]["coordinates"]
        self.org_lon = float(self.organization_coords[0])
        self.org_lat = float(self.organization_coords[1])

        self.temp_coord = self.lot_temp, self.lan_temp

        # Проверяем, что найденный объект не дальше 50м.
        print(self.temp_coord, (self.org_lon, self.org_lat))
        print(geospn.lonlat_distance(self.temp_coord, (self.org_lon, self.org_lat)))
        if geospn.lonlat_distance(self.temp_coord, (self.org_lon, self.org_lat)) <= 50:
            try:
                self.label_3.setText(f'Организация: {self.organization_name}')
            except:
                self.label_3.setText('Нет организации')
    ...
    def keyPressEvent(self, event):
        if event.key() == Qt.Key_PageUp and self.zoom < 18:
            ...
            self.coords = [self.coords[0], self.coords[1] + 0.008 * math.pow(2, 13 - self.zoom)]
            if self.flag_metka:
                self.draw_map_2()
            else:
                self.draw_map()
            ...
            self.coords = [self.coords[0] - 0.02 * math.pow(2, 13 - self.zoom), self.coords[1]]
            if self.flag_metka:
                self.draw_map_2()
            else:
                self.draw_map()
